...
# This priority queue is best implemented with a min-heap.
# pop: O(logn), sort: O(n), insert: O(logn)
# For a BST,
# pop: O(logn), sort: O(nlogn), insert: O(logn)
# For this array implementation:
# pop: O(n), sort: O(nlogn), insert: O(1)
# Only pop and insert are used because the queue starts
# off with zero length, and the implementation of pop and
# insert are such that a sort operation is not needed for
# the data structure.
...
class PriorityQueue:
    """Implementation of the priority queue using a min heap.

    This will reduce the time complexity from the array implementation
    of the priority queue from theta(n) to theta(logn).

    The required operations are the same as the above array implementation,
    namely append, min and pop.
    """
    ...
    def pop(self):
        # O(1)
        # Experiment with just doing array.pop(0) instead of the 3 operations below.
        min = self.array[0]
        self.array[0] = self.array[-1]
        self.array.pop()
        if len(self.array) > 0:
            # Cannot run min_heapify on an empty array.
            self._min_heapify(1) # Note that the heap functions always use the [1, n] index.
        return min

    def append(self, key):
        # log(n) since only need to traverse the height of the tree, with each
        # execution of the body inside the while O(1).
    ...
    def _min_heapify(self, i):
        """The most important function for a min heap. This is log(n).

        If the ith element of the heap has both its subtrees as minheaps, then this function
        will convert the subtree rooted at element i into a minheap.

        Note that the indices have to be adjusted for the [0, n) convention in programming.
        """
        # Store values to reduce array index calculation and array accessing.
        key = self.array[i - 1]
        left_child = self.array[i * 2 - 1] if i * 2 - 1 < len(self.array) else None
        right_child = self.array[i * 2] if i * 2 < len(self.array) else None
        
        # The logic below which covers the conditions for swapping is exhaustive because
        # either the left is less than the node or not, and if the left is less than
        # the node, either the right is less than the left or not, and if the left is
        # not less than the node, then either the right is less than the node or not.
        # All other cases does not necessitate a swapping condition.
        # Lastly, this recurses down to the node for which the swapping occurs if it occurs, because
        # that smaller subtree rooted at the child for which the swapping occurs, due
        # to the big element which might have come down from node i could have lost its
        # minheap property, since it is our assumption that we do not know what the value of 
        # node i is.
        if left_child and left_child < key:
            if right_child and right_child < left_child:
                # case 1: right child < left child < node
                self.array[i - 1] = right_child
                self.array[i * 2] = key
                self._min_heapify(i * 2 + 1)
            else:
                # case 2: left child <= right child; left child < node
                self.array[i - 1] = left_child
                self.array[i * 2 - 1] = key
                self._min_heapify(i * 2)
        elif right_child and right_child < key:
            # case 3: right child < node <= left node
            self.array[i - 1] = right_child
            self.array[i * 2] = key
            self._min_heapify(i * 2 + 1)
